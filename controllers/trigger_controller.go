...
func (r *TriggerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := controllerutil.SetControllerReference(trigger, triggerDeployment, r.Scheme); err != nil {
	...
	if !reflect.DeepEqual(trigger.Spec.Replicas, found.Spec.Replicas) {
		found.Spec.Replicas = trigger.Spec.Replicas
...
func (r *TriggerReconciler) getDeploymentForTrigger(trigger *vanusv1.Trigger) *appsv1.Deployment {
	ls := labelsForTrigger(trigger.Name)
	...
	dep := &appsv1.Deployment{
		...
		Spec: appsv1.DeploymentSpec{
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					ServiceAccountName: trigger.Spec.ServiceAccountName,
...
func getEnvForTrigger(controller *vanusv1.Trigger) []corev1.EnvVar {
	envs := []corev1.EnvVar{{
...
func getVolumesForTrigger(trigger *vanusv1.Trigger) []corev1.Volume {
